...
echo $DEP__OphidiaTerminal__oph_term -j -u $PROP__User_Name -p ... -H $PROP__Server_Address -P $PROP__Server_Port -e "oph_aggregate operation=$PROP__Operation;group_size=$PROP__Group_Size;ncores=$PROP__Core_No_;objkey_filter=$PROP__Object_Key_Filter;${Cube}${Grid}${SessionId}"

STDOUT=$( mktemp inc-stdout-XXXX.txt )

$DEP__OphidiaTerminal__oph_term -j -u $PROP__User_Name -p $PROP__User_Password -H $PROP__Server_Address -P $PROP__Server_Port -e "oph_merge operation=$PROP__Operation;group_size=$PROP__Group_Size;ncores=$PROP__Core_No_;objkey_filter=$PROP__Object_Key_Filter;${Cube}${Grid}${SessionId}" > $STDOUT
