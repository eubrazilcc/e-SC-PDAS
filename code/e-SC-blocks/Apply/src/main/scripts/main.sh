...
if [ -z "$PROP__Dimensions_Query" ] ; then DimQuery="" ; else DimQuery="dim_query=$PROP__Dimensions_Query;" ; fi
if [ -z "$PROP__Check_Type" ] ; then CheckType="no" ; else CheckType="yes" ; fi
if [ -z "$PROP__Measure" ] ; then Measure="" ; else Measure="measure=$PROP__Measure;" ; fi
if [ -z "$PROP__Session_Id" ] ; then SessionId="" ; else SessionId="sessionid=$PROP__Session_Id;" ; fi

echo Running command:
echo $DEP__OphidiaTerminal__oph_term -j -u $PROP__User_Name -p ... -H $PROP__Server_Address -P $PROP__Server_Port -e "oph_apply query=$PROP__Query;$measure_type=$PROP__Measure_Type;dim_type=$PROP__Dimension_Type;check_type=$CheckType;compressed=$PROP__Compressed;ncores=$PROP__Core_No_;objkey_filter=$PROP__Object_Key_Filter;${Cube}${DimQuery}${Measure}${SessionId}"
