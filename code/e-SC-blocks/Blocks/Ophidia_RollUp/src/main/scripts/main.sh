...
OUTFILE=$(mktemp rollup-XXXX.csv)
echo $OUTFILE >> $OUTPUT__output_files

FILE=(rollup-*.csv)

for f in $BLOCK_HOME/scripts/rollup-*.csv; do
...
$DEP__OphidiaTerminal__oph_term oph_term -j -u $1 $PROP__u -p $2 $PROP__p -H $3 $PROP__H -P $4 $PROP__P -e "oph_rollup cube=$PROP__cube" | tee -a "$OUTFILE"

else
echo "Operation is not completed."
fi
done
